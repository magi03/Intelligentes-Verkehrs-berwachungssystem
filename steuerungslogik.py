...
ROIS_COUNTING = { 
    ...
    1: np.array([[287, 478], [497, 476], [418, 344], [285, 351]], np.int32), 

    2: np.array([[214, 233], [285, 225], [285, 279], [196, 283]], np.int32), 

    3: np.array([[520, 243], [560, 273], [382, 305], [373, 271]], np.int32), 

    4: np.array([[1, 312], [1, 348], [153, 360], [182, 318]], np.int32) 

} 

ROIS_VIOLATION = { 

    1: np.array([[314, 419], [441, 414], [315, 419], [442, 413]], np.int32), 

    2: np.array([[274, 348], [187, 352], [187, 352], [276, 349]], np.int32), 

    3: np.array([[383, 377], [358, 343], [384, 377], [357, 346]], np.int32), 

    4: np.array([[192, 385], [192, 426], [193, 390], [192, 426]], np.int32) 
...
ROIS_CRASH = { 
    ...
    1: np.array([[157, 360], [417, 348], [372, 267], [196, 279]], np.int32), 
...
def update_physical_lights(ns_state, ew_state): 
    ...
    GPIO.output(NS_RED, GPIO.HIGH if ns_state == "Rot" else GPIO.LOW) 
    GPIO.output(NS_YELLOW, GPIO.HIGH if ns_state == "Gelb" else GPIO.LOW) 
    GPIO.output(NS_GREEN, GPIO.HIGH if ns_state == "Grün" else GPIO.LOW) 

    GPIO.output(EW_RED, GPIO.HIGH if ew_state == "Rot" else GPIO.LOW) 
    GPIO.output(EW_YELLOW, GPIO.HIGH if ew_state == "Gelb" else GPIO.LOW) 
    ...
    while True: 
        ...
        # AMBULANZ LOGIK FÜR NORD-SÜD (Spur 1 & 2) 
            ...
            if current_phase == 2:      # Ost-West hat Grün -> Zwinge auf Gelb 
                current_phase = 3 
                last_switch = now 
            elif current_phase == 3:    # Ost-West ist Gelb -> Warte 2 Sek, dann NS Grün! 
        ...
        # AMBULANZ LOGIK FÜR OST-WEST (Spur 3 & 4) 
            ...
            if current_phase == 0:      # Nord-Süd hat Grün -> Zwinge auf Gelb 
                current_phase = 1 
                last_switch = now 
            elif current_phase == 1:    # Nord-Süd ist Gelb -> Warte 2 Sek, dann EW Grün! 
                if elapsed > YELLOW_TIME: 
                    current_phase = 2 
                    last_switch = now 
            ...
            if current_phase == 0: 
                ...
                if elapsed > YELLOW_TIME: current_phase = 2; last_switch = now 
